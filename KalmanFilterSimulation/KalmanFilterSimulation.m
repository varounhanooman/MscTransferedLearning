...
for i=1:num_time_step-1                     %Do this num_time_step times
    ...
    z(i+1) = normrnd(A*z(i), abs(sigma_z));  %perform state evolution on hidden value (equation 4)
    sigma_x = normrnd(0,sig_rand);          %choose new sigma_x from a normal distribution of mean zero and with variance sig_rand
    x(i+1) = normrnd(H*z(i+1), abs(sigma_x));%observe hidden value using observational model (equation 3)
end
